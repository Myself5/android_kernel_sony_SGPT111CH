...
static struct snd_kcontrol_new tegra_board_controls[] = {
	/* DAC mono mix control */
	SOC_SINGLE("DAC Mono Switch nbx", WM8903_DAC_DIGITAL_1, 12, 1, 0),

	/* DAC enable control */
	SOC_SINGLE("DACL nbx", WM8903_POWER_MANAGEMENT_6, 3, 1, 0),
	SOC_SINGLE("DACR nbx", WM8903_POWER_MANAGEMENT_6, 2, 1, 0),

	/* Headphone Amp Volume controls */
			      ...
			      int device_new, int is_call_mode_new)
{
	...
	if (play_device_new != audio_data->play_device) {
		codec_con &= ~(TEGRA_HEADPHONE | TEGRA_LINEOUT |
                       TEGRA_SPK | TEGRA_EAR_SPK | TEGRA_HEADSET | TEGRA_MONO_SPK);
		...
		if (play_device_new & TEGRA_AUDIO_DEVICE_OUT_MONO_SPEAKER)
			codec_con |= TEGRA_MONO_SPK;
...
int tegra_controls_init(struct snd_soc_codec *codec)
{
	...
	/* Add nbx specific controls */
	err = snd_soc_add_controls(codec, tegra_board_controls,
					ARRAY_SIZE(tegra_board_controls));
	if (err < 0)
		return err;

	err = volume_limit_dev_register(codec); /* For volume limitation */
	if (err < 0) {
		pr_err("Failed in volume limit init \n");
		return err;
	}

	/* End of nbx specific controls */
