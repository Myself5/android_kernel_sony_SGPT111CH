					...
					struct snd_pcm_hw_params *params)
{
	...
	snd_soc_write(codec, WM8903_POWER_MANAGEMENT_1, (WM8903_MIXOUTL_ENA | WM8903_MIXOUTR_ENA));
        tegra_das_power_mode(true, tegra_audio_codec_type_hifi);
        tegra_das_power_mode(true, tegra_audio_codec_type_bluetooth);
					...
					struct snd_pcm_hw_params *params)
{
    ...
    //        tegra_das_power_mode(false, tegra_audio_codec_type_hifi);
    //        tegra_das_power_mode(true, tegra_audio_codec_type_bluetooth);
...
{
        disable_irq(gpio_to_irq(tegra_wired_jack_conf.hp_det_n));
        disable_irq(gpio_to_irq(tegra_wired_jack_conf.cdc_shrt));
	return 0;
}
...
{
        enable_irq(gpio_to_irq(tegra_wired_jack_conf.hp_det_n));
        enable_irq(gpio_to_irq(tegra_wired_jack_conf.cdc_shrt));
...
void tegra_ext_control(struct snd_soc_codec *codec, int new_con)
{
	...
	if (new_con & (TEGRA_LINEOUT | TEGRA_EAR_SPK | TEGRA_HEADPHONE
			 | TEGRA_SPK | TEGRA_MONO_SPK | TEGRA_INT_MIC))
	...
	if (new_con & TEGRA_MONO_SPK)
		snd_soc_dapm_enable_pin(codec, "Mono Spk");
	else
		snd_soc_dapm_disable_pin(codec, "Mono Spk");

	if (new_con & TEGRA_INT_MIC) {
		if (tegra_wired_jack_conf.echo_canceler_wake)
			tegra_wired_jack_conf.echo_canceler_wake();
		snd_soc_dapm_enable_pin(codec, "Int Mic");
	}else{
		if (tegra_wired_jack_conf.echo_canceler_sleep)
			tegra_wired_jack_conf.echo_canceler_sleep();
...
{
        struct snd_soc_codec *codec_dev = w->codec;
...
#ifdef CONFIG_SND_SOC_WM8903_CHSW
	if (!ext_audio_get_hp_state() && SND_SOC_DAPM_EVENT_ON(event)){
		ext_audio_channel_switch(true);
	}else{
		ext_audio_channel_switch(false);
	}
#endif
	if (SND_SOC_DAPM_EVENT_ON(event))
		msleep(500);
...
/* nbx machine dapm widgets */
static const struct snd_soc_dapm_widget tegra_nbx_dapm_widgets[] = {
	SND_SOC_DAPM_SPK("Mono Spk", tegra_dapm_event_int_spk),
};

static const struct snd_soc_dapm_route audio_map_nbx[] = {
	/* build-in speaker connected to LON/P for mono output */
	{"Mono Spk", NULL, "LON"},
	{"Mono Spk", NULL, "LOP"},
};
...
static int tegra_codec_init(struct snd_soc_codec *codec)
{
	...
	if (!audio_data->init_done) {
		/* WORK AROUND: get struct clk by clock name defined directly here */
		audio_data->dap_mclk = tegra_get_clock_by_name("clk_dev1");
		...
		/* Add nbx specific widgets */
		snd_soc_dapm_new_controls(codec, tegra_nbx_dapm_widgets,
					ARRAY_SIZE(tegra_nbx_dapm_widgets));

		/* Set up tegra specific audio path audio_map */
		snd_soc_dapm_add_routes(codec, audio_map,
					ARRAY_SIZE(audio_map));

		/* Set up nbx specific audio path audio_map */
		snd_soc_dapm_add_routes(codec, audio_map_nbx,
					ARRAY_SIZE(audio_map_nbx));

#ifdef CONFIG_SND_SOC_WM8903_CHSW
		/* Add rotation detection */
		channel_switch_dev_register(&tegra_snd_device->dev);
#endif
...
{
#ifdef CONFIG_SND_SOC_WM8903_CHSW
        channel_switch_dev_unregister();
#endif
        volume_limit_dev_unregister(); /* initialized in tegra_controls_init */
	tegra_jack_exit();
