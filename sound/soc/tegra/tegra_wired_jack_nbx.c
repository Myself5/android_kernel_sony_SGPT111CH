...
struct wired_jack_conf tegra_wired_jack_conf = {
        -1, -1, -1, -1, 0, 0, NULL, NULL, 0, NULL, NULL,
};
...
void tegra_switch_set_state(int state)
{
	...
	ext_audio_set_hp_state(state);
#endif
	switch_set_state(&wired_switch_dev, state);
	hook_switch_is_headset(state == BIT_HEADSET ? 1 : 0);
}
...
static enum headset_state get_headset_state(void)
{
	...
	if (tegra_wired_jack_conf.cdc_shrt != -1)
		shrt_gpio = gpio_get_value(tegra_wired_jack_conf.cdc_shrt);

	pr_debug("hp_gpio:%d, mic_gpio:%d\n", hp_gpio, mic_gpio);

        mic_gpio ^= shrt_gpio;	/* 4-pole headset detection */
...
static int tegra_wired_jack_probe(struct platform_device *pdev)
{
	...
	tegra_wired_jack_conf.hp_det_n = hp_det_n;
        tegra_wired_jack_conf.cdc_shrt = cdc_shrt;
	tegra_wired_jack_conf.cdc_irq = cdc_irq;
	tegra_wired_jack_conf.en_spkr = en_spkr;

	/* restore callback to control echo canceler IC */
	if (pdata->echo_canceler_sleep && pdata->echo_canceler_wake){
		tegra_wired_jack_conf.echo_canceler_sleep = pdata->echo_canceler_sleep;
		tegra_wired_jack_conf.echo_canceler_wake = pdata->echo_canceler_wake;
	}
