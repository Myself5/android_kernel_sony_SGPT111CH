...
static int tegra_sdhci_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (host->card_always_on && is_card_sdio(host->sdhci->mmc->card)) {
		...
		/*
		 * Added to call the suspend functions of Atheros driver
		 */
		ret = mmc_suspend_host(host->sdhci->mmc);
#endif
...
static int tegra_sdhci_resume(struct platform_device *pdev)
{
	...
	if (host->card_always_on && is_card_sdio(host->sdhci->mmc->card)) {
		...
		//return 0;
		/*
		 * Added to call the resume functions of the Atheros driver
		 */
		return mmc_resume_host(host->sdhci->mmc);
#else
		return 0;
#endif
	}
