...
static inline signed int value_to_temperature(bool extended, u8 value)
{
	return ( extended ?
				( (signed int)value - (signed int)EXTENDED_RANGE_OFFSET ) :
				  (signed int)value );
}
...
static int __devinit nct1008_configure_sensor(struct nct1008_data* data)
{
	...
	value = temperature_to_value(pdata->ext_range, pdata->throttling_local_limit);
