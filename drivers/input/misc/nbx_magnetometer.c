...
static void nbx_magnetometer_set_active(struct nbx_magnetometer_data *mag, s8 is_active);
static void nbx_magnetometer_start_measurement(struct nbx_magnetometer_data *magnetometer);
static void nbx_magnetometer_stop_measurement(struct nbx_magnetometer_data *magnetometer);
...
{
	struct nbx_magnetometer_data *mag = i2c_get_clientdata(g_client);
...
static int read_data(struct nbx_magnetometer_data *mag, s32* px, s32* py, s32* pz)
{
	s32 err;
	s32 x, y, z;
 ...
 */
#if defined(CONFIG_INPUT_NBX_MAG_AXES_XZY)
	{
		s32 t;
		t = y;
		y = z;
...
#elif defined(CONFIG_INPUT_NBX_MAG_AXES_YZX)
	{
		s32 t;
		t = x;
		x = y;
...
#elif defined(CONFIG_INPUT_NBX_MAG_AXES_YXZ)
	{
		s32 t;
		t = x;
		x = y;
...
#elif defined(CONFIG_INPUT_NBX_MAG_AXES_ZXY)
	{
		s32 t;
		t = x;
		x = z;
...
#elif defined(CONFIG_INPUT_NBX_MAG_AXES_ZYX)
	{
		s32 t;
		t = x;
		x = z;
 ...
 */
#ifdef CONFIG_INPUT_NBX_MAG_INVERSE_X_AXIS
	x = -x;
#endif
#ifdef CONFIG_INPUT_NBX_MAG_INVERSE_Y_AXIS
	y = -y;
#endif
#ifdef CONFIG_INPUT_NBX_MAG_INVERSE_Z_AXIS
	z = -z;
#endif
...
static void timer_func(unsigned long arg)
{
	struct nbx_magnetometer_data *mag = (struct nbx_magnetometer_data*)arg;
...
static void nbx_magnetometer_start_measurement(struct nbx_magnetometer_data *mag)
{
	pr_debug("called\n");
	_i2c_write_byte(g_client, REG_CNTL1, 0x82);
...
static void nbx_magnetometer_stop_measurement(struct nbx_magnetometer_data *mag)
{
	pr_debug("called\n");
...
static void nbx_magnetometer_set_active(struct nbx_magnetometer_data *mag, s8 is_active)
{
	...
	if (is_active) {
		/* POWER ON */
		set_power(1);
		nbx_magnetometer_start_measurement(mag);
		mag->is_active = 1;
	} else {
		mag->is_active = 0;
		nbx_magnetometer_stop_measurement(mag);
		/* POWER OFF */
		set_power(0);
	}
	return ;
...
static void do_calibration(struct nbx_magnetometer_data *mag)
{
	int i;
	int k;
	s32 data;
...
static int nbx_magnetometer_open(struct inode *inode, struct file* filp)
{
	struct nbx_magnetometer_data *mag = i2c_get_clientdata(g_client);
	int retval;
...
static int nbx_magnetometer_release(struct inode *inode, struct file* filp)
{
	struct nbx_magnetometer_data *mag = i2c_get_clientdata(g_client);
	int retval;
	...
	pr_debug("called: %d\n", mag->open_count);
	{
		...
		if (mag->open_count < 1) {
			mag->open_count = 0;
			nbx_magnetometer_set_active(mag, 0);
		}
	}
	up(&mag->sem);
out:
...
static long nbx_magnetometer_ioctl(
		struct file* filp, unsigned int cmd, unsigned long arg)
{
	...
	retval = -ENOTTY;
	if (_IOC_TYPE(cmd) != NBX_MAGNETOMETER_IOC_MAGIC) {
		goto out;
	}
	if (_IOC_NR(cmd) >= NBX_MAGNETOMETER_IOC_MAXNR) {
		goto out;
	}
...

	{
		...
		s16 calib_data[6];
		switch (cmd) {
			case NBX_MAGNETOMETER_IOC_GET_ACTIVE:
				n = mag->is_active;
				pr_debug("NBX_MAGNETOMETER_IOC_GET_ACTIVE: %d\n", n);
				retval = copy_to_user(
				...
				break;
			case NBX_MAGNETOMETER_IOC_SET_ACTIVE:
				retval = copy_from_user(
							&n, (void __user*)arg, sizeof(n));
				if (retval != 0) {
					goto out_locked;
				}
				pr_debug("NBX_MAGNETOMETER_IOC_SET_ACTIVE: %d, %d\n",
							n, mag->is_active);
				nbx_magnetometer_set_active(mag, n);
				break;
			case NBX_MAGNETOMETER_IOC_DO_CALIBRATION:
				prev = mag->is_active;
				nbx_magnetometer_set_active(mag, 0);
				do_calibration(mag);
				...
				nbx_magnetometer_set_active(mag, prev);
				if (retval != 0) {
					goto out_locked;
				}
				break;
			case NBX_MAGNETOMETER_IOC_GET_OFS_DATA:
				...
				pr_debug("NBX_MAGNETOMETER_IOC_GET_OFS_DATA: "
							"0x%04x, 0x%04x, 0x%04x\n",
							(u16)mag->offset[0],
							(u16)mag->offset[1],
							(u16)mag->offset[2]);
				break;
			case NBX_MAGNETOMETER_IOC_SET_OFS_DATA:
				retval = copy_from_user(
				...
				pr_debug("NBX_MAGNETOMETER_IOC_SET_OFS_DATA: "
							"0x%04x, 0x%04x, 0x%04x\n",
							(u16)mag->offset[0],
							(u16)mag->offset[1],
							(u16)mag->offset[2]);
				nbx_magnetometer_set_active(mag, prev);
				break;
			case NBX_MAGNETOMETER_IOC_GET_B0_DATA:
				...
				pr_debug("NBX_MAGNETOMETER_IOC_GET_B0_DATA: "
							"0x%04x, 0x%04x, 0x%04x\n",
							(u16)mag->b0[0],
							(u16)mag->b0[1],
							(u16)mag->b0[2]);
				break;
			case NBX_MAGNETOMETER_IOC_SET_B0_DATA:
				retval = copy_from_user(
							data, (void __user*)arg, sizeof(data));
				if (retval != 0) {
					goto out_locked;
				...
				pr_debug("NBX_MAGNETOMETER_IOC_SET_B0_DATA: "
							"0x%04x, 0x%04x, 0x%04x\n",
							(u16)mag->b0[0],
							(u16)mag->b0[1],
							(u16)mag->b0[2]);
				nbx_magnetometer_set_active(mag, prev);
				break;
			case NBX_MAGNETOMETER_IOC_GET_DELAY:
				retval = copy_to_user(
							(void __user*)arg, &mag->delay, sizeof(mag->delay));
				if (retval != 0) {
					goto out_locked;
				}
				pr_debug("NBX_MAGNETOMETER_IOC_GET_DELAY: %d\n", mag->delay);
				break;
			case NBX_MAGNETOMETER_IOC_SET_DELAY:
				retval = copy_from_user(
							&n, (void __user*)arg, sizeof(n));
				if (retval != 0) {
					goto out_locked;
				...
				prev = mag->is_active;
				nbx_magnetometer_set_active(mag, 0);
				pr_debug("NBX_MAGNETOMETER_IOC_SET_DELAY: %d\n", mag->delay);
				nbx_magnetometer_set_active(mag, prev);
				break;
			default:
				goto out_locked;
...
static struct file_operations nbx_magnetometer_fops = {
	.owner = THIS_MODULE,
	.open = nbx_magnetometer_open,
	.release = nbx_magnetometer_release,
	.unlocked_ioctl = nbx_magnetometer_ioctl,
};

static struct miscdevice nbx_magnetometer_dev = {
	MISC_DYNAMIC_MINOR,
	"nbx_magnetometer",
	&nbx_magnetometer_fops,
};


static int nbx_magnetometer_probe(struct i2c_client *client,
					const struct i2c_device_id *id)
{
	s32 data;
	int err;
	int retval;
	struct nbx_magnetometer_data *mag;
	...
	mag->input_dev->name = "nbx_magnetometer";
	...
	err = misc_register(&nbx_magnetometer_dev);
	if (err) {
...
static int nbx_magnetometer_remove(struct i2c_client *client)
{
	struct nbx_magnetometer_data *mag = i2c_get_clientdata(client);

	pr_debug("called\n");
	misc_deregister(&nbx_magnetometer_dev);
	input_unregister_device(mag->input_dev);
...
static int nbx_magnetometer_suspend(struct i2c_client* client, pm_message_t state)
{
	int retval;
	struct nbx_magnetometer_data *mag = i2c_get_clientdata(g_client);
...
static int nbx_magnetometer_resume(struct i2c_client* client)
{
	int retval;
	struct nbx_magnetometer_data *mag = i2c_get_clientdata(g_client);
	...
	{
		if (mag->open_count > 0) {
			nbx_magnetometer_set_active(mag, mag->is_active);
		}
	}
	up(&mag->sem);
out:
...
MODULE_DEVICE_TABLE(i2c, nbx_magnetometer_idtable);

static struct i2c_device_id nbx_magnetometer_idtable[] = {
	{ "nbx_magnetometer", 0 },
	{ }
};

static struct i2c_driver nbx_magnetometer_driver = {
	.driver = {
	.name = "nbx_magnetometer",
	},
	.probe = nbx_magnetometer_probe,
	.remove = nbx_magnetometer_remove,

#ifdef CONFIG_PM
	.suspend = nbx_magnetometer_suspend,
	.resume = nbx_magnetometer_resume,
#endif /* CONFIG_PM */

	.id_table = nbx_magnetometer_idtable,
};

static int __init nbx_magnetometer_driver_init(void)
{
	int res;

	res = i2c_add_driver(&nbx_magnetometer_driver);

	return res;
}

static void __exit nbx_magnetometer_driver_exit(void)
{
	i2c_del_driver(&nbx_magnetometer_driver);
}

module_init(nbx_magnetometer_driver_init);
module_exit(nbx_magnetometer_driver_exit);
