...
#endif
#ifdef ENABLE_DETECT_SETTING
#include <linux/platform_device.h>
#endif /* ENABLE_DETECT_SETTING */
...
#endif
#ifdef ENABLE_DETECT_SETTING
	struct platform_device	hdmi_setting_dev;
#endif /* ENABLE_DETECT_SETTING */
...
#ifdef ENABLE_DETECT_SETTING
#ifdef CONFIG_SWITCH
	switch_set_state(&hdmi->detect_switch, 1);
#endif
	if (!hdmi->setting_output_enable) {
		goto fail;
	}
#endif /* ENABLE_DETECT_SETTING */
...
fail_no_det:
#ifdef CONFIG_SWITCH
	switch_set_state(&hdmi->detect_switch, 0);
#endif
	/* fall through */
#endif /* ENABLE_DETECT_SETTING */
...
{
	struct tegra_dc_hdmi_data *hdmi =
			container_of(dev_get_drvdata(dev), struct tegra_dc_hdmi_data, hdmi_setting_dev);

	if (hdmi) {
		return snprintf(buf, PAGE_SIZE, "%d\n", hdmi->setting_output_enable);
	} else {
		return 0;
	}
}
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (hdmi) {
		...
		switch (val) {
		case 0:
			hdmi->setting_output_enable = false;
			break;
		case 1:
			hdmi->setting_output_enable = true;
			break;
		default:
			return count;;
		}
		queue_work(system_nrt_wq, &hdmi->work);
		return count;
	}
	else {
		return 0;
...
static DEVICE_ATTR(output_enable, S_IRUSR, output_enable_show, output_enable_store);
static struct attribute *setting_attrs[] = {
	&dev_attr_output_enable.attr,
	NULL,
};
static struct attribute_group setting_attr_group = {
	.name = "setting",
	.attrs = setting_attrs,
};
#endif /* ENABLE_DETECT_SETTING */
...
#ifdef ENABLE_DETECT_SETTING
	hdmi->detect_switch.name = "_hdmi_det";
	ret = switch_dev_register(&hdmi->detect_switch);
#endif /* ENABLE_DETECT_SETTING */
#endif

#ifdef ENABLE_DETECT_SETTING
	hdmi->hdmi_setting_dev.name = "hdmi";
	hdmi->hdmi_setting_dev.id = -1;
	ret = platform_device_register(&hdmi->hdmi_setting_dev);
	if (!ret) {
		platform_set_drvdata(&hdmi->hdmi_setting_dev, &hdmi->hdmi_setting_dev);
		ret = sysfs_create_group(&hdmi->hdmi_setting_dev.dev.kobj,
			&setting_attr_group);
		WARN(ret, "could not create setting_attr_group\n");
	}
...
#ifdef ENABLE_DETECT_SETTING
	platform_device_unregister(&hdmi->hdmi_setting_dev);
#endif /* ENABLE_DETECT_SETTING */
#ifdef CONFIG_SWITCH
#ifdef ENABLE_DETECT_SETTING
	switch_dev_unregister(&hdmi->detect_switch);
#endif /* ENABLE_DETECT_SETTING */
