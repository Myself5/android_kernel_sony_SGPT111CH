...
struct tegra_ehci_hcd {
	...
	/* Necessary to avoid concurrency problems between tegra_ehci_irq
	 * and tegra_ehci_hub_control. Using ehci->lock does not work
	 * because it must be released before calling the ehci counterpart,
	 * which creates opportunity for a race condition. */
	spinlock_t lock;
};
...
static irqreturn_t tegra_ehci_irq (struct usb_hcd *hcd)
{
	...
	/* Start resume signaling if remote wakeup detected */
	status = ehci_readl(ehci, &ehci->regs->status);
	if (status & STS_PCD && !pmc_remote_wakeup) {
		int pstatus;

		if (ehci->has_ppcd && !((status >> 16) & (1 << 0)))
			goto end;
		...
		if (!(test_bit(0, &ehci->suspended_ports) &&
				((pstatus & PORT_RESUME) ||
...
)
{
	...
	if (typeReq == ClearPortFeature && wValue == USB_PORT_FEAT_ENABLE) {
		...
		if (tegra->port_resuming) {
			...
			/*
			 * On Tegra, PORT_RESUME bit will clear itself to zero
			 * once resume signal is over, hence ehci_hub_control
			 * can not catch the end-of-resume condition. Do its
			 * job here.
			 */
