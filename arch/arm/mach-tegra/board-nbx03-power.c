...
static struct regulator_init_data sm0_data = REGULATOR_INIT(sm0, 1000, 1200, ON, NULL); /* VDD_CORE */
static struct regulator_init_data sm1_data = REGULATOR_INIT(sm1, 750, 1050, ON, &sm1_config); /* VDD_CPU */
static struct regulator_init_data sm2_data = REGULATOR_INIT(sm2, 3000, 4550, ON, NULL);
static struct regulator_init_data ldo0_data = REGULATOR_INIT(ldo0, 1200, 1200, OFF, NULL);
static struct regulator_init_data ldo1_data = REGULATOR_INIT(ldo1, 1100, 1100, ON, NULL);
static struct regulator_init_data ldo2_data = REGULATOR_INIT(ldo2, 1000, 1200, OFF, NULL); /* VDD_RTC */
static struct regulator_init_data ldo3_data = REGULATOR_INIT(ldo3, 2700, 2700, OFF, NULL);
static struct regulator_init_data ldo4_data = REGULATOR_INIT(ldo4, 1800, 1800, OFF, NULL);
static struct regulator_init_data ldo5_data = REGULATOR_INIT(ldo5, 2850, 2850, ON, NULL);
static struct regulator_init_data ldo6_data = REGULATOR_INIT(ldo6, 3300, 3300, OFF, NULL); /* not used and should be disabled */
static struct regulator_init_data ldo7_data = REGULATOR_INIT(ldo7, 2850, 2850, OFF, NULL);
...
static struct i2c_board_info __initdata nbx03_regulators[] = {
	{
...
static struct tegra_suspend_platform_data nbx03_suspend_data = {
	/*
...
static irqreturn_t
vbus_intr(int irq, void *arg)
{
	struct platform_device *pdev = (struct platform_device *)arg;
	uint8_t stats;

        /* check USBDET */
	if (tps6586x_read(pdev->dev.parent, TPS6586X_STATS, &stats) < 0) {
		printk("tps6586x_read failed\n");
		goto err;
	}

	if (stats & 0x04) {
#ifndef NBX03_USBHOST_WAKELOCK
		/* if vbus out by myself(host mode), ignore */
		if (gpio_get_value(TEGRA_GPIO_PD0))
			return IRQ_HANDLED;
#endif
		wake_lock(&vbus_lock);
	} else {
#ifdef NBX03_USBHOST_WAKELOCK
		wake_lock_timeout(&vbus_lock, 5 * HZ);
#else
...
{
	tps6586x_set_bits(pdev->dev.parent, TPS6586X_PPATH1, 0x01);
	return 0;
}

static int vbusmon_resume(struct platform_device *pdev)
{
	tps6586x_clr_bits(pdev->dev.parent, TPS6586X_PPATH1, 0x01);
	return 0;
}
#endif

static int __devinit vbusmon_probe(struct platform_device *pdev)
{
	int irq, ret;

	wake_lock_init(&vbus_lock, WAKE_LOCK_SUSPEND, "vbusmon");
...
int __init nbx03_regulator_init(void)
{
	...
	if (minor == 3 && !(tegra_spare_fuse(18) || tegra_spare_fuse(19)))
		nbx03_suspend_data.suspend_mode = TEGRA_SUSPEND_LP1;
	...
	i2c_register_board_info(3, nbx03_regulators, 1);

	/* FIXME: adjust all always_on parameters before comment-in */
        //regulator_has_full_constraints();

	tegra_init_suspend(&nbx03_suspend_data);

	platform_driver_register(&vbusmon_driver);
