...
#else
static int das_set_pin_state(bool normal, enum tegra_audio_codec_type codec_type);
#endif
...
#else
int tegra_das_power_mode(bool is_normal, enum tegra_audio_codec_type codec_type)
{
        das_set_pin_state(is_normal, codec_type);
	return 0;
}
EXPORT_SYMBOL_GPL(tegra_das_power_mode);
#endif
...
static int tegra_das_probe(struct platform_device *pdev)
{
	...
	clk_enable(tegra_get_clock_by_name(das_drv_data->pdata->dap_clk));
